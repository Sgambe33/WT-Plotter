...
QJsonObject Utils::getJsonFromResources(const QString& resourceName, const QString& identifier) {
	...
	if (!file.open(QIODevice::ReadOnly)) {
		LOG_WARN_GLOBAL("Failed to open file: " + resourceName);
		return QJsonObject();
	}

	const QJsonDocument jsonDoc = QJsonDocument::fromJson(file.readAll());
	if (!jsonDoc.isArray()) {
		LOG_WARN_GLOBAL("Failed to parse JSON array from file: " + resourceName);
		return QJsonObject();
	}
	...
	for (const QJsonValue& value : jsonDoc.array()) {
		if (value.isObject()) {
			...
			if (obj.value("identifier").toString() == identifier) {
				return obj;
			}
		}
	}

	LOG_WARN_GLOBAL("No object found with identifier: " + identifier);
	return QJsonObject();
}
