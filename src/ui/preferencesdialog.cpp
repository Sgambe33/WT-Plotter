...
{
	const QString folderPath = QFileDialog::getExistingDirectory(this, tr("Select Replay Folder"), QString(), QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
...
{
	const QString folderPath = QFileDialog::getExistingDirectory(this, tr("Select Plot Save Path"), QString(), QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
	if (!folderPath.isEmpty()) {
...
void PreferencesDialog::loadLanguages()
{
	...
	const QString currentLanguage = settings.value("language", "en").toString();
	const int index = ui->languageComboBox->findData(currentLanguage);
	if (index >= 0) {
...
{
	const QString languageCode = ui->languageComboBox->itemData(index).toString();
