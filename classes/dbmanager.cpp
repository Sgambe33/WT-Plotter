...
QMap<QDate, QList<Replay>> DbManager::fetchReplaysGroupedByDate() {
    ...
    query.prepare(R"(
        SELECT session_id, author_id, start_time, map, game_mode, difficulty, status, time_played
        FROM Replay
    ...
    for (auto& replays : replayMap) {
        std::sort(replays.begin(), replays.end(),
                  [](const Replay& a, const Replay& b) { return a.getStartTime() < b.getStartTime(); });
    }
...
Replay DbManager::getReplayBySessionId(QString sessionId) {
    ...
    query.prepare(
        R"(SELECT * FROM PlayerReplayData JOIN Player ON PlayerReplayData.player_id=Player.player_id WHERE session_id = :session_id)");
    query.bindValue(":session_id", sessionId);
